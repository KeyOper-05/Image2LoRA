"""Transformer attention blocks for the hypernetwork decoder (from Video2LoRA)."""
...
class Attention(nn.Module):
    def __init__(
        ...
        self.scale = nn.Parameter(torch.ones(self.memory_layout[2](heads))) if cosine_attn else None
    ...
    def forward(self, x, context=None, mask=None):
        ...
        out = F.scaled_dot_product_attention(q.contiguous(), k.contiguous(), v.contiguous(), mask)
    ...
    def __init__(
        ...
        if not single_attn:
            ...
            self.attn2 = Attention(
                dim, context_dim, n_heads, d_head, self_cross, single_kv_head, cosine_attn, qk_head_ch
            )
